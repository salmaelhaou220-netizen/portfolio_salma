...
const PRODUCTIONS = [
  ...
  { tag: "Rapport",    tagCls: "",                                                      icon: "📄", title: "Projet de Fin de Formation (PFF)",     date: "Mai 2025", desc: "Mémoire de recherche sur l'apport des TICE dans l'enseignement de l'informatique au secondaire qualifiant au Maroc." },
];
...
export default function Productions() {
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        <h2 className="font-serif text-3xl font-bold mb-1" style={{ color: "#3D2B35" }}>Productions Réalisées</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {PRODUCTIONS.map(({ tag, tagCls, icon, title, date, desc }) => (
          <div key={title}
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "#E8B4C8"; }}
          ...
          >
            {tagCls ? (
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg inline-block mb-4"
                style={{ background: "#F5E6EE", color: "#C084A0", border: "1px solid #E8B4C8" }}>{tag}</span>
            )}
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-medium">
              <Calendar size={11} style={{ color: "#C084A0" }} /> {date}
            </div>
          </div>
        ))}
      </div>
