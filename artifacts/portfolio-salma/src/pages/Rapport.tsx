...
function StatCard({ icon: Icon, value, label, color, bg }: {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-xl p-6 text-center shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div className={`font-serif text-4xl font-bold mb-1 ${color}`}><span ref={ref}>0</span></div>
...
const CARDS = [
  { icon: ClipboardList, title: "Fiches de Préparation", desc: "Séances structurées selon les compétences du programme officiel, avec objectifs, déroulement et évaluation formative intégrée.", color: "text-blue-600", bg: "bg-blue-50" },
  { icon: Eye, title: "Grilles d'Observation", desc: "Outils d'évaluation par compétences permettant une observation structurée des pratiques pédagogiques lors des séances.", color: "text-indigo-600", bg: "bg-indigo-50" },
  { icon: Presentation, title: "Supports de Cours", desc: "Présentations interactives, tutoriels et ressources numériques développés pour faciliter l'apprentissage des élèves.", color: "text-violet-600", bg: "bg-violet-50" },
  { icon: FileText, title: "Projet de Fin de Formation", desc: "Mémoire de recherche synthétisant l'ensemble du parcours de stage et proposant des axes d'amélioration pédagogique.", color: "text-sky-600", bg: "bg-sky-50" },
];
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <StatCard icon={FileText}      value={1} label="Projet fin de formation" color="text-sky-600"    bg="bg-sky-50" />
      </div>
